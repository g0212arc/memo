...
- 冒頭に総額と1本あたりの平均
- 総合ランキングの各モデルに実費の列
- **費用の内訳**セクション … プロンプト別／モデル×プロンプトのマトリクス／高くついた出力の上位10本

1本ごとの正確な金額は `results/runs.json` の `cost_usd` にあります
（OpenRouter が実際に課金した額で、こちらの概算ではありません）。
為替レートは**実行時に取得**します（open.er-api.com）。取得できなかったときだけ既定値を使い、
レポートには使ったレートと取得時刻が明記されます。`--jpy-rate` で固定もできます。
決め打ちにすると必ず古くなります（実際に1日で 159.09 → 156.02 と動きました）。
...
学習に使う提供元があれば、点検の最後にまとめて警告します。

OpenRouter のアカウント設定（[Privacy](https://openrouter.ai/settings/privacy)）には
「学習する提供元へのルーティングを許すか」の設定が**有料モデル用と無料モデル用で別々**にあります。
これを許可しないと使えないモデル（`*-contributor` 系や一部の本家エンドポイント）があります。

**`models.txt` で提供元を固定していれば、この設定を許可に変えても他のモデルの行き先は変わりません。**
許可が要るモデルだけが解禁されます。
